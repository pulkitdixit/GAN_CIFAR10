...
class discriminator(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(196*1*1, 1) #output 1 value
...
class generator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
                ...
                nn.ReLU(),
                nn.BatchNorm2d(196)) #output image = 196*8*8
        ...
        self.conv5 = nn.Sequential(
                nn.ConvTranspose2d(196, 196, kernel_size = 4, stride = 2, padding = 1),
                nn.ReLU(),
                nn.BatchNorm2d(196)) #output image = 196*16*16
        ...
        self.conv7 = nn.Sequential(
                nn.ConvTranspose2d(196, 196, kernel_size = 4, stride = 2, padding = 1),
                nn.ReLU(),
                nn.BatchNorm2d(196)) #output image = 196*32*32
        
        self.conv8 = nn.Sequential(
                nn.Conv2d(196, 3, kernel_size = 3, stride = 1, padding = 1),
                nn.Tanh()) #output image = 3*32*32
...
for epochs in range(num_epochs):
    ...
    fig = plot(samples)
    plt.savefig('output/%s.png' % str(epochs).zfill(3), bbox_inches='tight')
    plt.close(fig)
